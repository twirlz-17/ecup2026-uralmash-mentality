...
| файл / папка | что это |
|---|---|
| `inference/` | **точный код контейнера**, который дал 0.54141 |
...
| `docs/WEIGHTS.md` | манифест весов с sha256 (веса передаются отдельно, 2.9 ГБ) |
...
Веса кросс-энкодеров (2.9 ГБ) в git не лежат — см. `docs/WEIGHTS.md`.

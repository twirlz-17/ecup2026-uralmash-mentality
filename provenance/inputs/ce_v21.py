...
def _build_texts(items_path, log):
    ...
    text = text.str.slice(0, int(os.environ.get("TEXT_CHAR_CAP", "900")))
    ids = df["id"].to_numpy()
...
def _score(model, tokenizer, t1, t2, device, batch_size, max_len, deadline_ts, log):
    """Returns (scores, complete). complete=False means the clock fired; the
    caller must discard the array (unscored positions are NaN)."""
    import torch
    ...
    lengths = np.fromiter(
        (len(a) + len(b) for a, b in zip(t1, t2)), dtype=np.int64, count=n
    )
    order = np.argsort(lengths, kind="stable")
    t1 = t1[order]
    t2 = t2[order]
    log("ce: pairs sorted by length")

    def tokenize(i):
        return tokenizer(
            list(t1[i:i + batch_size]),
            list(t2[i:i + batch_size]),
            padding=True,
            truncation=True,
            max_length=max_len,
            return_tensors="pt",
    ...
    threads = [threading.Thread(target=producer, daemon=True) for _ in range(n_prod)]
    for th in threads:
        th.start()
    ...
    with torch.inference_mode():
        while True:
            ...
            out = torch.sigmoid(logits.float()).cpu().numpy()
            ...
            if deadline_ts is not None and done >= batch_size * 4:
                ...
                if projected > deadline_ts:
                    log(f"ce: ABORT at {done}/{n} — projected finish "
                        f"{projected - deadline_ts:.0f}s past deadline "
                        f"({rate:.0f} pairs/s)")
                    complete = False
                    break
            if done % (batch_size * 50) < batch_size:
                log(f"ce: scored {done}/{n} "
                    f"({done / max(time.time() - t_start, 1e-6):.0f}/s)")

    if not complete:  # drain so producers exit promptly
        for qq in (work, q):
            try:
                while True:
                    qq.get_nowait()
    ...
    for th in threads:
        th.join(timeout=5)
    log(f"ce: scored {done}/{n} complete={complete}")
...
def ce_scores(items_path, matches_path, model_path, batch_size, max_len,
    ...
    _key = (items_path, matches_path, os.environ.get("TEXT_CHAR_CAP", "900"))
